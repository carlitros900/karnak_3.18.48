...
// jerry
extern s32 gtp_send_dock_cfg(struct i2c_client *client) ;
/*******************************************************
Function:
...
static s32 gup_init_panel(struct goodix_ts_data *ts)
{
	...
	GTP_DEBUG_FUNC();
		...
		printk("---lgx---gtupdate---001----dock_mode=%d\n",dock_mode);
		if(dock_mode)
		{
			printk("---lgx---gtupdate---001ib----dock_mode=%d\n",dock_mode);
			ret = gtp_send_dock_cfg(ts->client);
		}
		else
		{
			printk("---lgx---gtupdate---001il----dock_mode=%d\n",dock_mode);
...
update_fail:
	...
	if (SUCCESS == update_ret) {
		...
		if(dock_mode)
		{
			printk("---lgx---gtupdate---002ib----dock_mode=%d\n",dock_mode);
			ret = gtp_send_dock_cfg(i2c_connect_client);
		}
		else
		{
			printk("---lgx---gtupdate---002il----dock_mode=%d\n",dock_mode);
