...
u8 dock_config[GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH]
	= {GTP_REG_CONFIG_DATA >> 8, GTP_REG_CONFIG_DATA & 0xff};
u8 tmp_dock_config[GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH]
	= {GTP_REG_CONFIG_DATA >> 8, GTP_REG_CONFIG_DATA & 0xff};

u8 dock_mode;
static u8  pre_mode = 0 ;
...
s32 gtp_send_dock_cfg(struct i2c_client *client)
{
	s32 ret = 2;
#if GTP_DRIVER_SEND_CFG
	s32 retry = 0;
	...
	GTP_INFO("Second:Driver send dock config.");
	for (retry = 0; retry < 5; retry++) {
		ret = gtp_i2c_write(client, dock_config, GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH);
		if (ret > 0)
			break;
	}
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
	...
	ret = gtp_parse_dt_cfg(&ts->client->dev, &dock_config[GTP_ADDR_LENGTH], &ts->gtp_cfg_len, 2);
	ret = gtp_parse_dt_cfg(&ts->client->dev, &tmp_dock_config[GTP_ADDR_LENGTH], &ts->gtp_cfg_len, 0);
	...
	{
#if GTP_DRIVER_SEND_CFG
		printk("tp-dock_mode=%d\n",dock_mode);
                if (0x01 == dock_mode )
		{
			dock_mode = DOCK_MODE ;
		}else{
			dock_mode = NORMAL_MODE ;
		}
		if(dock_mode==DOCK_MODE)
		{
			pre_mode=DOCK_MODE;
			ret = gtp_send_dock_cfg(ts->client);
		}
		else
		{
			pre_mode=NORMAL_MODE;
			ret = gtp_send_cfg(ts->client);
		}
		printk("tp-dock_mode=%d,pre_mode=%d\n",dock_mode,pre_mode);
...
static ssize_t gt91xx_config_write_proc(struct file *filp, const char __user *buffer, size_t count, loff_t *off)
{
	...
	if(dock_mode){
		ret = gtp_send_dock_cfg(i2c_connect_client);
	}
	else{
...
static void gtp_esd_check_func(struct work_struct *work)
{
	...
	printk("esd0--dock_mode=%d,pre_mode=%d\n",dock_mode,pre_mode);
	if( dock_mode == DOCK_MODE )
	{
		if(pre_mode== NORMAL_MODE)
		{
			gtp_send_dock_cfg(i2c_connect_client);
		}
		pre_mode = DOCK_MODE;
	}
	else if ( dock_mode == NORMAL_MODE )
	{
	...
	printk("esd--dock_mode=%d,pre_mode=%d\n",dock_mode,pre_mode);
